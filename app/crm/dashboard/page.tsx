...
export default function Dashboard(){
 ...
 useEffect(()=>{
  Promise.all([fetch('/api/crm/auth/me'),fetch('/api/crm/contacts'),fetch('/api/crm/tasks'),fetch('/api/crm/appointments')]).then(async ([a,b,c,d])=>{
   if(a.status===401){window.location.href='/crm/login';return}
   const [ad,bd,cd,dd]=await Promise.all([a.json(),b.json(),c.json(),d.json()])
   setUser(ad.user);setContacts(bd.contacts||[]);setTasks(cd.tasks||[]);setAppointments(dd.appointments||[])
  })
 },[])
 ...
 return <main className="crm">
  <aside className="crm-sidebar">
   ...
   <nav className="crm-nav">{nav.map(({label,href,icon:Icon})=><a key={label} href={href} className={label==='Overview'?'active':''}><Icon size={17} strokeWidth={1.8}/><span>{label}</span>{label==='Conversations'&&<em>0</em>}</a>)}</nav>
