...
export default function Dashboard(){
 ...
 async function add(){
  ...
  if(r.ok){setContacts(p=>[d.contact,...p]);setShowAdd(false);setForm({firstName:'',lastName:'',email:'',phone:'',company:''})}else setAddError(d.error||'Customer could not be created.')
 }
 async function createReactivationTask(id:string){const r=await fetch('/api/crm/retention',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({contactId:id})});if(r.ok){setRetention(p=>p.filter(c=>c.id!==id));setTasks(p=>[...p,{id:'new-'+id,title:'Reactivation follow-up',first_name:'Customer',due_at:new Date().toISOString(),completed_at:null}])}else{const d=await r.json();alert(d.error||'Could not create follow-up')}}
 async function completeTask(id:string){
  const r=await fetch('/api/crm/tasks',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,completed:true})})
  if(r.ok){
   setTasks(previous=>previous.map(task=>task.id===id?{...task,completed_at:new Date().toISOString()}:task))
  }
 }
  ...
  <section className="crm-main">
   ...
   <div className="dashboard">
    ...
    <div className="dashboard-grid">
     ...
     <section className="panel attention-panel"><div className="panel-head"><div><span className="eyebrow">FOLLOW-UP</span><h3>Needs attention</h3></div><span className="count-badge">{openTasks}</span></div><div className="task-list">{tasks.filter(t=>!t.completed_at).slice(0,4).map(t=><div className="task" key={t.id}><button className="task-check" aria-label={'Complete '+t.title} onClick={()=>completeTask(t.id)}/><div><strong>{t.title}</strong><span>{[t.first_name,t.last_name].filter(Boolean).join(' ')||'General task'}</span></div><small>{t.due_at?new Date(t.due_at).toLocaleDateString():'No due date'}</small></div>)}{!openTasks&&<div className="soft-empty">No open follow-ups.</div>}</div></section>
    </div>
    <section className="panel attention-panel"><div className="panel-head"><div><span className="eyebrow">RETENTION</span><h3>Reactivation opportunities</h3></div><span className="count-badge">{retention.length}</span></div><div className="task-list">{retention.slice(0,4).map(c=><a key={c.id} className="task" href={"/crm/contacts?contact="+c.id}><div><strong>{c.first_name} {c.last_name||""}</strong><span>{c.company||c.email||"Customer"}</span></div><small>{c.last_activity_at?"Last activity "+new Date(c.last_activity_at).toLocaleDateString():"No activity"}</small><button className="text-action" onClick={e=>{e.preventDefault();createReactivationTask(c.id)}}>Create follow-up</button></a>)}{!retention.length&&<div className="soft-empty">No customers currently flagged for reactivation.</div>}</div></section><section className="panel leads-panel"><div className="panel-head"><div><span className="eyebrow">RECENT ACTIVITY</span><h3>Recent contacts</h3></div><a className="text-action" href="/crm/contacts">View all contacts</a></div><div className="lead-table"><div className="table-row table-head"><span>CONTACT</span><span>SOURCE</span><span>STATUS</span><span>CREATED</span><span/></div>{filtered.slice(0,8).map(c=><a className="table-row" key={c.id} href={"/crm/contacts?contact="+c.id}><span className="contact-cell"><b>{[c.first_name,c.last_name].filter(Boolean).map((x:string)=>x[0]).join('').toUpperCase()}</b><strong>{c.first_name} {c.last_name||''}<small>{c.company||c.email||'No details'}</small></strong></span><span>{c.source||'Manual'}</span><span><i className={'status-dot '+(c.status||'new')}/>{(c.status||'new')[0].toUpperCase()+(c.status||'new').slice(1)}</span><span>{new Date(c.created_at).toLocaleDateString()}</span><MoreHorizontal size={17}/></a>)}{!filtered.length&&<div className="soft-empty">No contacts yet. Add one or submit a lead from the public site.</div>}</div></section>
   </div>
  </section>
  {showAdd&&<div className="modal-overlay" onClick={()=>setShowAdd(false)}><div className="add-modal" onClick={e=>e.stopPropagation()}><button className="modal-close" onClick={()=>setShowAdd(false)}><X size={18}/></button><span className="eyebrow">NEW CUSTOMER</span><h2>Add a customer</h2><p>This creates a customer record in the current workspace.</p><div className="modal-form">{addError&&<div className="form-error">{addError}</div>}<input placeholder="First name" value={form.firstName} onChange={e=>setForm({...form,firstName:e.target.value})}/><input placeholder="Last name" value={form.lastName} onChange={e=>setForm({...form,lastName:e.target.value})}/><input placeholder="Email address" type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/><input placeholder="Phone number" value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})}/><input placeholder="Company or property" value={form.company} onChange={e=>setForm({...form,company:e.target.value})}/><button className="add-button" onClick={add} disabled={saving}>{saving?'Creating...':'Create customer'}</button></div></div></div>}
 </main>
}
