...
export async function GET(req:NextRequest){
 ...
 await ensureSchema()
 const sql=getDb()
 await sql`update automation_jobs set status='pending',last_error='Recovered stale running job',run_at=now() where status='running' and run_at<now()-interval '15 minutes'`
