...
const statements = [
  ...
  `create table if not exists conversations (id uuid primary key default gen_random_uuid(), organization_id uuid not null references organizations(id) on delete cascade, contact_id uuid not null references contacts(id) on delete cascade, channel text not null, status text not null default 'open', created_at timestamptz not null default now(), updated_at timestamptz not null default now())`,
  `create table if not exists messages (id uuid primary key default gen_random_uuid(), organization_id uuid not null references organizations(id) on delete cascade, conversation_id uuid not null references conversations(id) on delete cascade, direction text not null, body text not null, sent_at timestamptz not null default now())`,
  ...
  `insert into organizations (name, slug, industry, primary_color) values ('Fullerton Automotive', 'fullerton-automotive', 'Automotive', '#B8E03E') on conflict (slug) do nothing`,
  `insert into pipeline_stages (organization_id, name, position) select id, 'New', 1 from organizations where slug='fullerton-automotive' on conflict (organization_id, name) do nothing`,
  `insert into pipeline_stages (organization_id, name, position) select id, 'Contacted', 2 from organizations where slug='fullerton-automotive' on conflict (organization_id, name) do nothing`,
  `insert into pipeline_stages (organization_id, name, position) select id, 'Qualified', 3 from organizations where slug='fullerton-automotive' on conflict (organization_id, name) do nothing`,
  `insert into pipeline_stages (organization_id, name, position) select id, 'Won', 4 from organizations where slug='fullerton-automotive' on conflict (organization_id, name) do nothing`
]

export async function GET() {
  const sql = getDb()
  for (const statement of statements) await sql.unsafe(statement)
  const rows = await sql`select id, name, slug from organizations where slug='fullerton-automotive'`
