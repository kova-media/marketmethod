...
export async function POST(req:NextRequest){await ensureSchema();const s=getSession();if(!s)return NextResponse.json({error:'Unauthorized'},{status:401});const b=await req.json();if(!b.name||!b.triggerType)return NextResponse.json({error:'Name and trigger are required'},{status:400});
...
for(const action of b.actions){
 ...
 if(action.type==='send_email'&&(!String(action.subject||'').trim()||!String(action.body||'').trim()))return NextResponse.json({error:'Email action requires a subject and message'},{status:400});
 if(action.type==='send_sms'&&!String(action.body||'').trim())return NextResponse.json({error:'SMS action requires a message'},{status:400});
}
