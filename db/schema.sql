-- Canonical CRM schema bootstrap. Runtime uses lib/schema.ts.
-- This file is a SQL snapshot for inspection and manual provisioning.
...
create table if not exists login_attempts (id uuid primary key default gen_random_uuid(), email text not null, attempted_at timestamptz not null default now(), successful boolean not null default false);
...
alter table user_invites add column if not exists token_hash text;

create unique index if not exists user_invites_token_hash_idx on user_invites(token_hash) where token_hash is not null;
